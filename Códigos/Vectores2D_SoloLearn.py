import math
import time  # Importing mathematics and time modules
...
def sumavec(f1, theta1, f2, theta2):
    try:
        ...
        if Fr != 0:
            ...
            if frx > 0:
                if fry < 0:
                    # calculating angle using  absolute values of the
                    # components
                    polar = 360 - angle
                else:
                    polar = angle
        ...
        # Rounding angles very close to  360 degrees and set it to zero degrees
        if round(polar, 4) == 360:
            polar = 0

        print("Resulting vector\nMagnitude:{x} Polar Angle:{y}".format(
            x=round(Fr, 4), y=round(polar, 4)))
    ...
    while F1 == 'x' and t1 == 'x':
        try:
            ...
            if F1 < 0:
                print('Something goes wrong...')
                time.sleep(2)
                # Rasing a Value Error when a negative number is input
                raise ValueError
